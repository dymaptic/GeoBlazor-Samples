...
5. To speed up local development, we have added settings in `csproj`, `App.razor`, and `Program.cs` that 
   ...
   build pipeline or local environment.
    ...
    ```bash
    dotnet restore
    dotnet build SimpleSearch.csproj --configuration Release --no-restore /p:ENABLE_COMPRESSION=true
    dotnet publish SimpleSearch.csproj --configuration Release --no-restore --no-build /p:ENABLE_COMPRESSION=true
    ```
