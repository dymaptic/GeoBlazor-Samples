...
namespace dymaptic.GeoBlazor.Pro.Sample.Maui.Platforms.Android;
...
internal class PermissionManagingBlazorWebChromeClient : WebChromeClient, IActivityResultCallback
{
    ...
    private static readonly Dictionary<string, string> sRationalesByPermission = new()
    {
        [Manifest.Permission.AccessFineLocation] = LocationAccessRationale,
        // Add more rationales as you add more supported permissions.
    };

    private static readonly Dictionary<string, string[]> sRequiredPermissionsByWebkitResource = new()
    {
    ...
    private void RequestAllResources(Memory<string> requestedResources, Action<List<string>> callback)
    {
        ...
        var requiredPermissions = sRequiredPermissionsByWebkitResource.GetValueOrDefault(currentResource, Array.Empty<string>());
    ...
    private void RequestPermission(string permission, Action<bool> callback)
    {
        ...
        else if (_activity.ShouldShowRequestPermissionRationale(permission) && sRationalesByPermission.TryGetValue(permission, out var rationale))
        {
